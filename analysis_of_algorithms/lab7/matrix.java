...
public class matrix
{
	public static void main(String [] args)
	{
		...
		System.out.println();
		  double[][] A = {{1, 1, 1},
	        ...
	        double[][] res = strassenForOddNumbers(A, B, A.length);
	        
	        
	        
	        System.out.println("multiplication of odd matices:");
    ...
    public static double[][] normalMultiplication(double[][] A, double[][] B) {
    ...
    public static double[][] strassenForOddNumbers(double[][] A, double[][] B, int n) {
        double log = Math.log(n) / Math.log(2);
        if(log != (int)Math.ceil(log)) {  
        ...
        if (n==1) {
            ...
            double[][] I = strassenForOddNumbers(matrixOpt(A12, A22, '-'), matrixOpt(B21, B22, '+'), n/2);    // I = (A12 - A22)(B21 + B22)
            double[][] II =  strassenForOddNumbers(matrixOpt(A11, A22, '+'), matrixOpt(B11, B22, '+'), n/2);  // II = (A11 + A22)(B11 + B22)
            double[][] III = strassenForOddNumbers(matrixOpt(A11, A21, '-'), matrixOpt(B11, B12, '+'), n/2);  // III = (A11 - A21)(B11 + B12)
            double[][] IV = strassenForOddNumbers(matrixOpt(A11, A12, '+'), B22, n/2);                        // IV = (A11 + A12)B22
            double[][] V =  strassenForOddNumbers(A11, matrixOpt(B12, B22, '-'), n/2);                        // V = A11(B12 - B22)
            double[][] VI = strassenForOddNumbers(A22, matrixOpt(B21, B11, '-'), n/2);                        // VI = A22(B21 - B11)
            double[][] VII = strassenForOddNumbers(matrixOpt(A21, A22, '+'), B11, n/2);                        // VII = (A21 + A22)B11
